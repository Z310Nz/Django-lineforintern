...
try:
    ...
    if hasattr(enum, "version"):
        ...
        from enum import (
            EnumMeta as StdlibEnumMeta,
            Enum as StdlibEnum,
            IntEnum as StdlibIntEnum,
        )

        StdlibFlag = StdlibIntFlag = StdlibStrEnum = StdlibReprEnum = None
        stdlib_enums = StdlibEnum, StdlibIntEnum
except ImportError:
    StdlibEnumMeta = StdlibEnum = StdlibIntEnum = StdlibIntFlag = StdlibFlag = (
        StdlibStrEnum
    ) = StdlibStrEnum = StdlibReprEnum = None
...
def export(collection, namespace=None):
    ...
    if namespace is None:
        ...
        issubclass(collection, Enum)
        or stdlib_enums
        and issubclass(collection, stdlib_enums)
    ):
    ...
    def __delete__(self, instance):
        if self.fdel is None:
            if self.fget or self.fset:
                raise AttributeError(
                    "cannot delete attribute %r on <aenum %r>"
...
class EnumConstants(NamedConstant):
    AddValue = constant(
        "addvalue", "prepends value(s) from _generate_next_value_ to each member"
    )
    MagicValue = constant(
        "magicvalue", "calls _generate_next_value_ when no arguments are given"
    )
        ...
        if enum_class._noalias_:
            ...
            try:
                try:
                    ...
                    for (
                        unhashable_value,
                        canonical_member,
                    ) in enum_class._value2member_seq_:
            ...
            if nonunique:
                ...
                for name, aliases in nonunique.items():
                    ...
                    message.append(
                        "%s --> %s [%r]" % (name, bad_aliases, enum_class[name].value)
                    )
                raise ValueError(
...
class EnumDict(dict):
    ...
    def __init__(
        ...
        if self._addvalue and self._magicvalue:
            raise TypeError(
                "%r: AddValue and MagicValue are mutually exclusive" % cls_name
            )
        if self._multivalue and self._magicvalue:
            raise TypeError(
                "%r: MultiValue and MagicValue are mutually exclusive" % cls_name
            )
        self._start = start
    ...
    def __getitem__(self, key):
        if key == self._cls_name and self._cls_name not in self:
            ...
            value = self._generate_next_value(
                key, self._start, len(self._member_names), self._last_values[:]
            )
    ...
    def __setitem__(self, key, value):
        ...
        if is_internal_class(self._cls_name, value):
            ...
            if key not in (
                ...
                raise ValueError(
                    "%r: _sunder_ names, such as %r, are reserved for future Enum use"
                    % (self._cls_name, key)
                )
                ...
                raise ValueError(
                    "%r: cannot set %r after init phase" % (self._cls_name, key)
                )
            elif key == "_ignore_":
                if self._ignore_init_done:
                    raise TypeError(
                        "%r: ignore can only be specified once" % self._cls_name
                    )
                ...
                if already:
                    raise ValueError(
                        "%r: _ignore_ cannot specify already set names %s"
                        % (self._cls_name, ", ".join(repr(a) for a in already))
                    )
                self._ignore_init_done = True
            elif key == "_boundary_":
                if self._constructor_boundary:
                    raise TypeError(
                        "%r: boundary specified in constructor and class body"
                        % self._cls_name
                    )
            elif key == "_start_":
                if self._constructor_start:
                    raise TypeError(
                        "%r: start specified in constructor and class body"
                        % self._cls_name
                    )
                self._start = value
            elif key == "_settings_":
                ...
                if NoAlias in value and Unique in value:
                    raise TypeError(
                        "%r: NoAlias and Unique are mutually exclusive" % self._cls_name
                    )
                elif MultiValue in value and NoAlias in value:
                    raise TypeError(
                        "cannot specify both MultiValue and NoAlias" % self._cls_name
                    )
                allowed_settings = dict.fromkeys(
                    ["addvalue", "magicvalue", "noalias", "unique", "multivalue"]
                )
                for arg in self._settings:
                    if arg not in allowed_settings:
                        raise TypeError(
                            "%r: unknown qualifier %r (from %r)"
                            % (self._cls_name, arg, value)
                        )
                ...
                if self._constructor_init:
                    raise TypeError(
                        "%r: init specified in constructor and in class body"
                        % self._cls_name
                    )
                _init_ = value
                ...
                if self._magicvalue:
                    raise TypeError(
                        "%r: _init_ and MagicValue are mutually exclusive"
                        % self._cls_name
                    )
                self._init = _init_
                value = _init_
                ...
                if value is not None:
                    if isinstance(value, staticmethod):
                        ...
                        raise TypeError(
                            "%r: _generate_next_value must be a staticmethod, not a classmethod"
                            % self._cls_name
                        )
                    else:
                        gnv = value
            ...
            if key in self:
                ...
                raise TypeError(
                    "%r: %s already defined as %r" % (self._cls_name, key, self[key])
                )
            if type(value) is enum:
                value.name = key
                if self._addvalue:
                    raise TypeError(
                        "%r: enum() and AddValue are incompatible" % self._cls_name
                    )
        ...
        if not (
            is_sunder(key)
            or is_dunder(key)
            or is_private_name(self._cls_name, key)
            or is_descriptor(value)
        ):
            if not auto_store:
                # reset for next pass
                auto_store = True
    ...
    def _gnv(self, key, value):
        ...
        if self._auto_args:
            if not isinstance(value, tuple):
                value = (value,)
            value = self._generate_next_value(
                key, self._start, len(self._member_names), self._last_values[:], *value
            )
        else:
            value = self._generate_next_value(
                key, self._start, len(self._member_names), self._last_values[:]
...
class EnumType(type):
    ...
    @classmethod
    def __prepare__(
        metacls, cls, bases, init=None, start=None, settings=(), boundary=None, **kwds
    ):
        ...
        if (
            Flag is not None
            and any(b is Flag for b in bases)
            and member_type not in (baseinteger + (object,))
        ):
            if Flag in bases:
    ...
    def __new__(
        ...
        **kwds
        ...
        clsdict["_boundary_"] = boundary or getattr(first_enum, "_boundary_", None)
        ...
        if (
            ...
            if [m._value_ for m in enum_class] != sorted(
                [m._value_ for m in enum_class]
            ):
            ...
            if _order_:
                ...
                _order_ = [
                    ...
                    or is_single_bit(enum_class[o]._value_)
                ]
        #
    ...
    def __setattr__(cls, name, value):
        ...
        elif isinstance(found_attr, (bltin_property, property)) and not isinstance(
            value, property
        ):
            raise AttributeError(
    ...
    def _convert(cls, *args, **kwds):
        ...
        warnings.warn(
            "_convert is deprecated and will be removed, use" " _convert_ instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def _create_(
        ...
        * A mapping of member name -> value.
        ...
        if clsdict is None:
            ...
            clsdict = metacls.__prepare__(class_name, bases)
        enum_class = metacls.__new__(
            metacls, class_name, bases, clsdict, boundary=boundary
        )
        ...
        for chain in bases:
            for base in chain.__mro__:
                if issubclass(base, Enum) and base._member_names_:
                    raise TypeError("<aenum %r> cannot extend %r" % (class_name, base))
...
if PY3:
    ...
    def __dir__(self):
        ...
        if self.__class__._member_type_ is object:
            ...
            interesting = set(
                n for n in object.__dir__(self) if n not in self._member_map_
            )
    ...
    def __new__(cls, *values, **kwds):
        ...
        if values:
            if not isinstance(values[0], str):
                raise TypeError(
                    "%s: values must be str [%r is a %r]"
                    % (cls.__name__, values[0], type(values[0]))
                )
        value = str(*values)
...
def extend_enum(enumeration, name, *args, **kwds):
    ...
    if (
        ...
        raise TypeError(
            "%r already in use as %r"
            % (name, enumeration.__dict__.get(name, enumeration[name]))
        )
    ...
    for base in enumeration.__mro__[1:]:
        ...
        if descriptor is not None:
            if isinstance(descriptor, (property, DynamicClassAttribute)):
                ...
                raise TypeError(
                    "%r already in use in superclass %r" % (name, base.__name__)
                )
    try:
    ...
    if _multi_value_ and not args:
        ...
        raise ValueError(
            "no values specified for MultiValue enum %r" % enumeration.__name__
        )
    ...
    if _no_alias_:
        # unless NoAlias was specified
        return _finalize_extend_enum(
            enumeration, new_member, bits=_all_bits_, mask=_flag_mask_
        )
    else:
        # handle "normal" aliases
        new_values = new_member._values_
        for canonical_member in _member_map_.values():
            canonical_values_ = getattr(
                canonical_member, "_values_", [canonical_member._value_]
            )
        ...
        if issubclass(enumeration, flag_bases) and hasattr(enumeration, "_all_bits_"):
            ...
            if is_single_bit(value):
                ...
                if (
                    value & enumeration._flag_mask_ == value
                    and _value2member_map_.get(value) is not None
                ):
...
def _finalize_extend_enum(
    enumeration, new_member, name=None, bits=None, mask=None, is_alias=False
):
...
def _create_pseudo_member_(cls, *values):
    ...
    if not isinstance(value, baseinteger):
        raise ValueError("%r is not a valid %s" % (error_value, cls.__name__))
    # check boundaries
