...
def test_pickle_dump_load(
    assertion, source, target=None, protocol=(0, HIGHEST_PROTOCOL)
):
    start, stop = protocol
    failures = []
        ...
        def tester(first, op, final, second=None):
            if second is None:
                ...
                self.assertEqual(
                    ...
                    final,
                    "forward: %r %s %r -> %r != %r"
                    % (first, op.__name__, second, value.value, final),
                )
                left = auto()
                right = second
                value = op(left, right)
                ...
                self.assertEqual(
                    ...
                    final,
                    "reversed: %r %s %r -> %r != %r"
                    % (second, op.__name__, first, value.value, final),
                )
        ...
        def tester(first, op, final, second=None):
            if second is None:
                ...
                if secondary.value != final:
                    errors.append(
                        "%s %r -> %r != %r"
                        % (op.__name__, first, secondary.value, final),
                    )
            else:
                left = constant(first)
                right = second
                ...
                if value != final:
                    errors.append(
                        "forward: %r %s %r -> %r != %r"
                        % (first, op.__name__, second, value, final),
                    )
                left = first
                ...
                if value != final:
                    errors.append(
                        "reversed: %r %s %r -> %r != %r"
                        % (second, op.__name__, first, value, final),
                    )
...
class TestEnumType(TestCase):
    ...
    def test_immutability(self):
        ...
        self.assertRaisesRegex(
            AttributeError, "cannot rebind property", setattr, Huh, "value", "boom"
        )
        self.assertRaisesRegex(
            AttributeError, "cannot delete property", delattr, Huh, "value"
        )
        self.assertRaisesRegex(
            AttributeError, "cannot set attribute", setattr, Huh.one, "value", "boom"
        )
        self.assertRaisesRegex(
            AttributeError, "cannot delete attribute", delattr, Huh.two, "value"
        )
    ...
    def test_enum_shadow_base(self):
        ...
        self.assertEqual(
            list(Color), [Color.red, Color.green, Color.blue, Color.cyan, Color.azure]
        )
        self.assertRaisesRegex(
            AttributeError, "no attribute .cyan.", lambda: Color.blue.cyan
        )
...
class TestEnum(TestCase):
    ...
    def test_format_enum(self):
        ...
        self.assertEqual(
            "{0:}".format(Season.SPRING), "{0:}".format(str(Season.SPRING))
        )
        self.assertEqual(
            "{0:20}".format(Season.SPRING), "{0:20}".format(str(Season.SPRING))
        )
        self.assertEqual(
            "{0:^20}".format(Season.SPRING), "{0:^20}".format(str(Season.SPRING))
        )
    ...
    def test_programatic_function_from_dict(self):
        SummerMonth = Enum(
            "SummerMonth", dict((("june", 1), ("july", 2), ("august", 3)))
        )
        lst = list(SummerMonth)
        self.assertEqual(len(lst), len(SummerMonth))
        self.assertEqual(len(SummerMonth), 3, SummerMonth)
    ...
    def test_programatic_function_unicode_list(self):
        SummerMonth = Enum(
            "SummerMonth", [unicode("june"), unicode("july"), unicode("august")]
        )
        lst = list(SummerMonth)
        self.assertEqual(len(lst), len(SummerMonth))
        self.assertEqual(len(SummerMonth), 3, SummerMonth)
    ...
    def test_programmatic_function_unicode_class(self):
        if PY2:
            class_names = unicode("SummerMonth"), "S\xfcmm\xe9rM\xf6nth".decode(
                "latin1"
            )
        else:
        ...
        for i, class_name in enumerate(class_names):
            if PY2 and i == 1:
                self.assertRaises(
                    TypeError, Enum, class_name, unicode("june july august")
                )
            else:
    ...
    def test_meta_reconfigure(self):
        ...
        class JSONEnumMeta(EnumMeta):
            ...
            def __new__(
                metacls, cls, bases, clsdict, init=None, start=None, settings=()
            ):
                import json
                ...
                enum_dict = super(JSONEnumMeta, metacls).__prepare__(
                    cls, bases, init, start, settings
                )
                ...
                for name, value in members:
                    enum_dict[name] = value
                return super(JSONEnumMeta, metacls).__new__(
                    metacls, cls, bases, enum_dict, init, start, settings
                )
        ...
        self.assertEqual(
            [Country.AF, Country.AX, Country.AL, Country.DZ], list(Country)
        )
    ...
    def test_inherited_new_from_enhanced_enum(self):
        ...
        self.assertEqual(
            len(Color),
            3,
            "wrong number of elements: %d (should be %d)" % (len(Color), 3),
        )
    ...
    def test_number_reset_and_order_cleanup(self):
        ...
        self.assertEqual(
            list(Confused), [Confused.ONE, Confused.TWO, Confused.THREE, Confused.FOUR]
        )
    ...
    def test_auto_and_kwds(self):
        ...
        self.assertEqual(
            (Item.A.value, Item.A.size, Item.A.req), (1, 100, {"red": True})
        )
        self.assertEqual(
            (Item.B.value, Item.B.size, Item.B.req), (2, 200, {"red": False})
        )
    ...
    def test_auto_and_init_w_value_and_too_many_values(self):
        with self.assertRaisesRegex(
            TypeError, r"Field\.BLAH: number of fields provided do not match init"
        ):
    ...
    def test_multiple_arg_auto(self):
        ...
        class Planet(AutoName):
            ...
            MERCURY = auto(3.303e23, 2.4397e6)
            VENUS = auto(4.869e24, 6.0518e6)
    ...
    def test_auto_w_multiple_arg(self):
        ...
        class Planet(AutoName):
            _init_ = "value mass radius"
            MERCURY = auto(), 3.303e23, 2.4397e6  # doesn't work
            VENUS = auto(), 4.869e24, 6.0518e6  # doesn't work
    ...
    def test_auto_gnv_and_init(self):
        ...
        class Planet(AutoName):
            _init_ = "value mass radius"
            MERCURY = 3.303e23, 2.4397e6  # doesn't work
            VENUS = 4.869e24, 6.0518e6  # doesn't work
    ...
    def test_constantness_of_constants(self):
        ...
        self.assertRaisesRegex(
            AttributeError, r"cannot rebind constant", setattr, Universe, "PI", 9
        )
        self.assertRaisesRegex(
            AttributeError, r"cannot delete constant", delattr, Universe, "PI"
        )
    ...
    def test_constant_with_auto_is_updated(self):
        ...
        self.assertEqual(
            list(Fruit), [Fruit.apple, Fruit.banana, Fruit.lemon, Fruit.orange]
        )
    ...
    def test_multiple_mixin(self):
        ...
        self.assertEqual(Color.MAX, 3)
        self.assertEqual(
            str(Color.BLUE), "blue", "%r is not %r" % (str(Color.BLUE), "blue")
        )
        ...
        self.assertEqual(
            str(CoolColor.BLUE), "blue", "%r is not %r" % (str(Color.BLUE), "blue")
        )
        ...
        self.assertEqual(
            str(CoolerColor.BLUE), "blue", "%r is not %r" % (str(Color.BLUE), "blue")
        )
        ...
        self.assertEqual(
            str(CoolestColor.BLUE), "blue", "%r is not %r" % (str(Color.BLUE), "blue")
        )
        ...
        self.assertEqual(
            str(ConfusedColor.BLUE), "blue", "%r is not %r" % (str(Color.BLUE), "blue")
        )
        ...
        self.assertEqual(
            str(ReformedColor.BLUE), "blue", "%r is not %r" % (str(Color.BLUE), "blue")
        )
...
class TestStrEnum(TestCase):
    ...
    def test_strict_strenum(self):
        ...
        self.assertEqual(
            [m.value for m in Either], ["this", "That", "those", "lower", "UPPER"]
        )
        #
        ...
        self.assertEqual(
            [m.value for m in Lower], ["this", "that", "those", "lower", "upper"]
        )
        #
        ...
        self.assertEqual(
            [m.value for m in Upper], ["THIS", "THAT", "THOSE", "LOWER", "UPPER"]
        )
...
class TestFlag(TestCase):
        ...
        class TermColor(str, Flag):
            ...
            def __repr__(self):
                if self._name_ is not None:
                    ...
                    return "<%s: %s>" % (
                        self.__class__.__name__,
                        "|".join([m.name for m in Flag.__iter__(self)]),
                    )
    ...
    def test_doc_flag(self):
        ...
        self.assertEqual(
            (AS.NAME | AS.STREET)._value_, 48, "%r is not 48" % (AS.NAME | AS.STREET)
        )
    ...
    def test_unique_composite(self):
        ...
        self.assertFalse(
            failed[0], "at least one thread failed while creating composite members"
        )
    ...
    def test_autovalue_and_generate_next_value(self):
        class Color(str, Flag):
            _order_ = (
                "FG_Black FG_Red FG_Green FG_Blue BG_Yellow BG_Magenta BG_Cyan BG_White"
            )
...
class TestNamedTuple(TestCase):
    ...
    def test_asdict(self):
        ...
        self.assertEqual(
            pixel._asdict(), {"x": 99, "y": -101, "r": 255, "g": 128, "b": 0}
        )
...
class TestExtendEnum(TestCase):
    ...
    def test_extend_enum_plain(self):
        ...
        self.assertRaisesRegex(
            TypeError, "already in use as", extend_enum, Color, "blue", 5
        )
        #
    ...
    def test_extend_enum_unique(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            r"<Color.rojo: 1> is a duplicate of <Color.red: 1>",
            extend_enum,
            Color,
            "rojo",
            1,
    ...
    def test_extend_enum_shadow_base(self):
        ...
        self.assertRaisesRegex(
            TypeError, r"already in use in superclass", extend_enum, Color, "cyan", 4
        )
        self.assertEqual(len(Color), 3)
        self.assertEqual(list(Color), [Color.red, Color.green, Color.blue])
    ...
    def test_extend_enum_multivalue(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            "no values specified for MultiValue enum",
            extend_enum,
            Color,
            "mauve",
        )
    ...
    def test_extend_enum_multivalue_alias(self):
        ...
        self.assertRaisesRegex(
            ValueError,
            r"<Color.rojo: 7> is a duplicate of <Color.red: 1>",
            extend_enum,
            Color,
            "rojo",
            7,
...
class TestIssues(TestCase):
    ...
    def test_auto_kwds(self):
        ...
        self.assertEqual(
            (Item.B.value, Item.B.size, Item.B.requirements), (2, 200, {Item.A: 1})
        )
    ...
    def test_auto_kwds_and_gnv(self):
        ...
        self.assertEqual(
            (Item.A.value, Item.A.size, Item.A.requirements), ("A", 100, {})
        )
        self.assertEqual(
            (Item.B.value, Item.B.size, Item.B.requirements), ("B", 200, {Item.A: 1})
        )
