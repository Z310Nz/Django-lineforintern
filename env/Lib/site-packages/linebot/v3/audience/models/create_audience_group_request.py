...
class CreateAudienceGroupRequest(BaseModel):
    ...
    description: Optional[constr(strict=True, max_length=120)] = Field(
        None,
    ...
    upload_description: Optional[StrictStr] = Field(
        None,
        alias="uploadDescription",
        description="The description to register for the job (in jobs[].description). ",
    )
    audiences: Optional[conlist(Audience, max_items=10000)] = Field(
        None, description="An array of user IDs or IFAs. Max number: 10,000 "
    )
        ...
        _obj = CreateAudienceGroupRequest.parse_obj(
            {
                ...
                "audiences": (
                    [Audience.from_dict(_item) for _item in obj.get("audiences")]
                    if obj.get("audiences") is not None
                    else None
                ),
            }
        )
