...
class CreateClickBasedAudienceGroupResponse(BaseModel):
    ...
    audience_group_id: Optional[StrictInt] = Field(
        None, alias="audienceGroupId", description="The audience ID."
    )
    type: Optional[AudienceGroupType] = None
    description: Optional[StrictStr] = Field(None, description="The audience's name.")
    created: Optional[StrictInt] = Field(
        None, description="When the audience was created (in UNIX time)."
    )
    ...
    click_url: Optional[StrictStr] = Field(
        None,
        alias="clickUrl",
        description="The URL that was specified when the audience was created.",
    )
    create_route: Optional[StrictStr] = Field(
        None,
        alias="createRoute",
        description="How the audience was created. `MESSAGING_API`: An audience created with Messaging API. ",
    )
    permission: Optional[StrictStr] = Field(
        None,
        description="Audience's update permission. Audiences linked to the same channel will be READ_WRITE.  - `READ`: Can use only. - `READ_WRITE`: Can use and update. ",
    )
    expire_timestamp: Optional[StrictInt] = Field(
        None,
        alias="expireTimestamp",
        description="Time of audience expiration. Only returned for specific audiences.",
    )
    is_ifa_audience: Optional[StrictBool] = Field(
        False,
        alias="isIfaAudience",
        description="The value indicating the type of account to be sent, as specified when creating the audience for uploading user IDs. One of:  true: Accounts are specified with IFAs. false (default): Accounts are specified with user IDs. ",
    )
        ...
        _obj = CreateClickBasedAudienceGroupResponse.parse_obj(
            {
                ...
                "is_ifa_audience": (
                    obj.get("isIfaAudience")
                    if obj.get("isIfaAudience") is not None
                    else False
                ),
            }
        )
