...
class GetAudienceDataResponse(BaseModel):
    ...
    jobs: Optional[conlist(AudienceGroupJob, max_items=50)] = Field(
        None,
        description="An array of jobs. This array is used to keep track of each attempt to add new user IDs or IFAs to an audience for uploading user IDs. Empty array is returned for any other type of audience. Max: 50 ",
    )
    ...
    def from_dict(cls, obj: dict) -> GetAudienceDataResponse:
        ...
        _obj = GetAudienceDataResponse.parse_obj(
            {
                "audience_group": (
                    AudienceGroup.from_dict(obj.get("audienceGroup"))
                    if obj.get("audienceGroup") is not None
                    else None
                ),
                "jobs": (
                    [AudienceGroupJob.from_dict(_item) for _item in obj.get("jobs")]
