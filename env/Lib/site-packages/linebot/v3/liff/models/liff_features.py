...
class LiffFeatures(BaseModel):
    ...
    ble: Optional[StrictBool] = Field(
        None,
        description="`true` if the LIFF app supports Bluetooth® Low Energy for LINE Things. `false` otherwise. ",
    )
    qr_code: Optional[StrictBool] = Field(
        False,
        alias="qrCode",
        description="`true` to use the 2D code reader in the LIFF app. false otherwise. The default value is `false`. ",
    )
        ...
        _obj = LiffFeatures.parse_obj(
            {
                "ble": obj.get("ble"),
                "qr_code": (
                    obj.get("qrCode") if obj.get("qrCode") is not None else False
                ),
            }
        )
        return _obj
