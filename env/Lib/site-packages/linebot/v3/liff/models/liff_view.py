...
class LiffView(BaseModel):
    ...
    type: StrictStr = Field(
        ...,
        description="Size of the LIFF app view. Specify one of these values: - compact - tall - full ",
    )
    url: StrictStr = Field(
        ...,
        description="Endpoint URL. This is the URL of the web app that implements the LIFF app (e.g. https://example.com). Used when the LIFF app is launched using the LIFF URL. The URL scheme must be https. URL fragments (#URL-fragment) can't be specified. ",
    )
    module_mode: Optional[StrictBool] = Field(
        None,
        alias="moduleMode",
        description="`true` to use the LIFF app in modular mode. When in modular mode, the action button in the header is not displayed. ",
    )
