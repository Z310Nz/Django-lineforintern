...
class CallbackRequest(BaseModel):
    ...
    destination: constr(strict=True, max_length=33, min_length=33) = Field(
        ...,
        description="User ID of a bot that should receive webhook events. The user ID value is a string that matches the regular expression, `U[0-9a-f]{32}`. ",
    )
    events: conlist(Event) = Field(
        ...,
        description="Array of webhook event objects. The LINE Platform may send an empty array that doesn't include a webhook event object to confirm communication. ",
    )
