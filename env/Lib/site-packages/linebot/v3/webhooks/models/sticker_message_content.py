...
class StickerMessageContent(MessageContent):
    ...
    keywords: Optional[conlist(StrictStr, max_items=15)] = Field(
        None,
        description="Array of up to 15 keywords describing the sticker. If a sticker has 16 or more keywords, a random selection of 15 keywords will be returned. The keyword selection is random for each event, so different keywords may be returned for the same sticker. ",
    )
    text: Optional[constr(strict=True, max_length=100)] = Field(
        None,
        description="Any text entered by the user. This property is only included for message stickers. Max character limit: 100 ",
    )
    quote_token: StrictStr = Field(
        ..., alias="quoteToken", description="Quote token to quote this message. "
    )
    quoted_message_id: Optional[StrictStr] = Field(
        None,
        alias="quotedMessageId",
    ...
    def sticker_resource_type_validate_enum(cls, value):
        ...
        if value not in (
            ...
            raise ValueError(
                "must be one of enum values ('STATIC', 'ANIMATION', 'SOUND', 'ANIMATION_SOUND', 'POPUP', 'POPUP_SOUND', 'CUSTOM', 'MESSAGE', 'NAME_TEXT', 'PER_STICKER_TEXT')"
            )
        return value
