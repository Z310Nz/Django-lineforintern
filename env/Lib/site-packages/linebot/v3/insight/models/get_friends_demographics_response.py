...
class GetFriendsDemographicsResponse(BaseModel):
    ...
    available: Optional[StrictBool] = Field(
        None, description="true if friend demographic information is available."
    )
    genders: Optional[conlist(GenderTile)] = Field(
        None, description="Percentage per gender."
    )
    ages: Optional[conlist(AgeTile)] = Field(
        None, description="Percentage per age group."
    )
    areas: Optional[conlist(AreaTile)] = Field(None, description="Percentage per area.")
    app_types: Optional[conlist(AppTypeTile)] = Field(
        None, alias="appTypes", description="Percentage by OS."
    )
    subscription_periods: Optional[conlist(SubscriptionPeriodTile)] = Field(
        None,
        alias="subscriptionPeriods",
        ...
        _obj = GetFriendsDemographicsResponse.parse_obj(
            {
                ...
                "genders": (
                    [GenderTile.from_dict(_item) for _item in obj.get("genders")]
                    if obj.get("genders") is not None
                    else None
                ),
                "ages": (
                    [AgeTile.from_dict(_item) for _item in obj.get("ages")]
                    if obj.get("ages") is not None
                    else None
                ),
                "areas": (
                    [AreaTile.from_dict(_item) for _item in obj.get("areas")]
                    if obj.get("areas") is not None
                    else None
                ),
                "app_types": (
                    [AppTypeTile.from_dict(_item) for _item in obj.get("appTypes")]
                    if obj.get("appTypes") is not None
                    else None
                ),
                "subscription_periods": (
                    [
                        SubscriptionPeriodTile.from_dict(_item)
                        for _item in obj.get("subscriptionPeriods")
                    ]
                    if obj.get("subscriptionPeriods") is not None
                    else None
                ),
            }
        )
