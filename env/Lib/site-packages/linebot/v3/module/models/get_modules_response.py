...
class GetModulesResponse(BaseModel):
    ...
    bots: conlist(ModuleBot) = Field(
        ...,
        description="Array of Bot list Item objects representing basic information about the bot.",
    )
    next: Optional[StrictStr] = Field(
        None,
        description="Continuation token. Used to get the next array of basic bot information. This property is only returned if there are more unreturned results. ",
    )
    ...
    def from_dict(cls, obj: dict) -> GetModulesResponse:
        ...
        _obj = GetModulesResponse.parse_obj(
            {
                "bots": (
                    [ModuleBot.from_dict(_item) for _item in obj.get("bots")]
                    if obj.get("bots") is not None
                    else None
                ),
                "next": obj.get("next"),
            }
