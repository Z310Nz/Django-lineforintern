...
class FlexBubble(FlexContainer):
    ...
    def size_validate_enum(cls, value):
        ...
        if value not in ("nano", "micro", "deca", "hecto", "kilo", "mega", "giga"):
            raise ValueError(
                "must be one of enum values ('nano', 'micro', 'deca', 'hecto', 'kilo', 'mega', 'giga')"
            )
        return value
        ...
        _obj = FlexBubble.parse_obj(
            {
                ...
                "styles": (
                    FlexBubbleStyles.from_dict(obj.get("styles"))
                    if obj.get("styles") is not None
                    else None
                ),
                "header": (
                    FlexBox.from_dict(obj.get("header"))
                    if obj.get("header") is not None
                    else None
                ),
                "hero": (
                    FlexComponent.from_dict(obj.get("hero"))
                    if obj.get("hero") is not None
                    else None
                ),
                "body": (
                    FlexBox.from_dict(obj.get("body"))
                    if obj.get("body") is not None
                    else None
                ),
                "footer": (
                    FlexBox.from_dict(obj.get("footer"))
