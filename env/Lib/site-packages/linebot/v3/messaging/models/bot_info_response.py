...
class BotInfoResponse(BaseModel):
    ...
    chat_mode: StrictStr = Field(
        ...,
        alias="chatMode",
        description='Chat settings set in the LINE Official Account Manager. One of:  `chat`: Chat is set to "On". `bot`: Chat is set to "Off". ',
    )
    mark_as_read_mode: StrictStr = Field(
        ...,
        alias="markAsReadMode",
        description='Automatic read setting for messages. If the chat is set to "Off", auto is returned. If the chat is set to "On", manual is returned.  `auto`: Auto read setting is enabled. `manual`: Auto read setting is disabled.  ',
    )
