...
class ReplyMessageResponse(BaseModel):
    ...
    sent_messages: conlist(SentMessage, max_items=5, min_items=1) = Field(
        ..., alias="sentMessages", description="Array of sent messages."
    )
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ReplyMessageResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ReplyMessageResponse:
        """Create an instance of ReplyMessageResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ReplyMessageResponse.parse_obj(obj)

        _obj = ReplyMessageResponse.parse_obj(
            {
                "sent_messages": (
                    [SentMessage.from_dict(_item) for _item in obj.get("sentMessages")]
                    if obj.get("sentMessages") is not None
                    else None
                )
            }
        )
