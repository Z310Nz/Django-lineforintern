...
class FlexImage(FlexComponent):
    ...
    url: StrictStr = Field(
        ...,
        description="Image URL (Max character limit: 2000) Protocol: HTTPS (TLS 1.2 or later) Image format: JPEG or PNG Maximum image size: 1024×1024 pixels Maximum file size: 10 MB (300 KB when the animated property is true) ",
    )
    flex: Optional[StrictInt] = Field(
        None,
        description="The ratio of the width or height of this component within the parent box.",
    )
    margin: Optional[StrictStr] = Field(
        None,
    ...
    position: Optional[StrictStr] = Field(
        None,
        description="Reference for offsetTop, offsetBottom, offsetStart, and offsetEnd. Specify one of the following values:  `relative`: Use the previous box as reference. `absolute`: Use the top left of parent element as reference. The default value is relative. ",
    )
    offset_top: Optional[StrictStr] = Field(
        None, alias="offsetTop", description="Offset."
    )
    offset_bottom: Optional[StrictStr] = Field(
        None, alias="offsetBottom", description="Offset."
    )
    offset_start: Optional[StrictStr] = Field(
        None, alias="offsetStart", description="Offset."
    )
    offset_end: Optional[StrictStr] = Field(
        None, alias="offsetEnd", description="Offset."
    )
    align: Optional[StrictStr] = Field(
        None, description="Alignment style in horizontal direction. "
    )
    gravity: Optional[StrictStr] = Field(
        None, description="Alignment style in vertical direction."
    )
    size: Optional[StrictStr] = Field(
        "md", description="The maximum image width. This is md by default. "
    )
    aspect_ratio: Optional[StrictStr] = Field(
        None,
        alias="aspectRatio",
        description="Aspect ratio of the image. `{width}:{height}` format. Specify the value of `{width}` and `{height}` in the range from `1` to `100000`. However, you cannot set `{height}` to a value that is more than three times the value of `{width}`. The default value is `1:1`. ",
    )
    aspect_mode: Optional[StrictStr] = Field(
        None,
        alias="aspectMode",
    ...
    animated: Optional[StrictBool] = Field(
        False,
        description="When this is `true`, an animated image (APNG) plays. You can specify a value of true up to 10 images in a single message. You can't send messages that exceed this limit. This is `false` by default. Animated images larger than 300 KB aren't played back. ",
    )
    type: str = "image"
        ...
        _obj = FlexImage.parse_obj(
            {
                ...
                "action": (
                    Action.from_dict(obj.get("action"))
                    if obj.get("action") is not None
                    else None
                ),
                "animated": (
                    obj.get("animated") if obj.get("animated") is not None else False
                ),
            }
        )
        return _obj
