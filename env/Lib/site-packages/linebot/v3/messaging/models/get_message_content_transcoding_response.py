...
class GetMessageContentTranscodingResponse(BaseModel):
    ...
    status: StrictStr = Field(
        ...,
        description="The preparation status. One of:  `processing`: Preparing to get content. `succeeded`: Ready to get the content. You can get the content sent by users. `failed`: Failed to prepare to get the content. ",
    )
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in ("processing", "succeeded", "failed"):
            raise ValueError(
                "must be one of enum values ('processing', 'succeeded', 'failed')"
            )
        return value
