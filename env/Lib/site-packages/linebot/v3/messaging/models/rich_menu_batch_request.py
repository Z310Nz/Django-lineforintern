...
class RichMenuBatchRequest(BaseModel):
    ...
    operations: conlist(RichMenuBatchOperation, max_items=1000) = Field(
        ..., description="Array of Rich menu operation object..."
    )
    resume_request_key: Optional[constr(strict=True, max_length=100, min_length=1)] = (
        Field(
            None,
            alias="resumeRequestKey",
    ...
    def resume_request_key_validate_regular_expression(cls, value):
        ...
        if not re.match(r"^[a-zA-Z0-9_-]{1,100}$", value):
            raise ValueError(
                r"must validate the regular expression /^[a-zA-Z0-9_-]{1,100}$/"
            )
        return value
        ...
        _obj = RichMenuBatchRequest.parse_obj(
            {
                "operations": (
                    [
                        RichMenuBatchOperation.from_dict(_item)
