...
class MembersIdsResponse(BaseModel):
    ...
    member_ids: conlist(StrictStr, max_items=100) = Field(
        ...,
        alias="memberIds",
    ...
    next: Optional[StrictStr] = Field(
        None,
        description="A continuation token to get the next array of user IDs of the members in the group chat. Returned only when there are remaining user IDs that were not returned in `memberIds` in the original request. ",
    )
    ...
    def from_dict(cls, obj: dict) -> MembersIdsResponse:
        ...
        _obj = MembersIdsResponse.parse_obj(
            {"member_ids": obj.get("memberIds"), "next": obj.get("next")}
        )
        return _obj
