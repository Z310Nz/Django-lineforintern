...
class GetWebhookEndpointResponse(BaseModel):
    ...
    active: StrictBool = Field(
        ...,
        description="Webhook usage status. Send a webhook event from the LINE Platform to the webhook URL only if enabled.  `true`: Webhook usage is enabled. `false`: Webhook usage is disabled. ",
    )
    ...
    def from_dict(cls, obj: dict) -> GetWebhookEndpointResponse:
        ...
        _obj = GetWebhookEndpointResponse.parse_obj(
            {"endpoint": obj.get("endpoint"), "active": obj.get("active")}
        )
        return _obj
