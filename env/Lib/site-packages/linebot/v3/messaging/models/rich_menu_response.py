...
class RichMenuResponse(BaseModel):
    ...
    rich_menu_id: StrictStr = Field(
        ..., alias="richMenuId", description="ID of a rich menu"
    )
    ...
    name: constr(strict=True, max_length=300) = Field(
        ...,
        description="Name of the rich menu. This value can be used to help manage your rich menus and is not displayed to users.",
    )
    chat_bar_text: constr(strict=True, max_length=14) = Field(
        ..., alias="chatBarText", description="Text displayed in the chat bar"
    )
    areas: conlist(RichMenuArea, max_items=20) = Field(
        ...,
        ...
        _obj = RichMenuResponse.parse_obj(
            {
                ...
                "size": (
                    RichMenuSize.from_dict(obj.get("size"))
                ...
                "areas": (
                    [RichMenuArea.from_dict(_item) for _item in obj.get("areas")]
