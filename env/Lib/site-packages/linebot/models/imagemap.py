...
class ImagemapSendMessage(SendMessage):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.base_size = self.get_or_new_from_json_dict(base_size, BaseSize)
        self.video = self.get_or_new_from_json_dict(video, Video)

        new_actions = []
        if actions:
            for action in actions:
                action_obj = self.get_or_new_from_json_dict_with_types(
                    action, {"uri": URIImagemapAction, "message": MessageImagemapAction}
                )
                if action_obj:
